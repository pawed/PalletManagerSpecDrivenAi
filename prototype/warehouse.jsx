...
function WarehouseSection({ lang, items }) {
  const t = I18N[lang];
  ...
  return (
    <>
      {/* Location chips — multi-select toggle */}
      ...
      <div className="filterbar">
        ...
        <MultiSelect
          ...
          options={FESTIVAL_DATA.WH_CATEGORIES.map(c => ({ value: c.id, label: c[lang] }))}
          lang={lang} />
        <div style={{ marginLeft: "auto", fontFamily: "var(--font-mono)", fontSize: 11.5, color: "var(--text-muted)" }}>
          {filtered.length} {lang === "pl" ? "pozycji" : "items"} · {totalQty} {lang === "pl" ? "szt łącznie" : "total"}
        </div>
      </div>

      <div className="table">
        <div className="table__head table__head--wh">
          <div>{t.name}</div>
        ...
        {filtered.length === 0 ? (
          ...
          return (
            <div key={it.id} className="table__row table__row--wh">
              <div style={{ fontWeight: 500 }}>{lang === "pl" ? it.name : it.nameEn}</div>
              <div className="table__amt" style={{ fontWeight: 600 }}>
                {it.qty}<span style={{ color: "var(--text-dim)", fontWeight: 400, marginLeft: 3 }}>{it.unit}</span>
              </div>
...
window.WarehouseSection = WarehouseSection;
